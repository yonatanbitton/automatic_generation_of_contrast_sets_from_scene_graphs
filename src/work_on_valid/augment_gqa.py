...
from config import scene_graphs_dir, questions_dir, number_of_augmentations_to_produce, experiment_output_dir, \
    experiment_output_name, data_dir
...
def main(stats_dict):
    balanced_val_data, val_scene_graphs = get_val_data()
...
def debug_single_vid(balanced_val_data, scene_graphs, all_qa_augmentations_df, chosen_image_id, qa_key):
    ...
    question = augmented_questions_for_image[1].iloc[0]['question']
    answer = augmented_questions_for_image[1].iloc[0]['answer']
    qa_augmentations = json.loads(augmented_questions_for_image[1].iloc[0]['augmented_qas'])[0]['qa_augmentations']
    draw_image_details(chosen_image_id, scene_graph_for_image_id, (question, answer), qa_augmentations, osp.join(data_dir, 'debug'))

    print(f"Finished debug {augmented_questions_for_image}")


def augment_qas_for_image_id(balanced_val_data, image_id, scene_graph_for_image_id, all_qa_augmentations_df, idx, skip_exists=True, debug_qa_key=None):
    questions_for_image_id = {question_key: question_dict for question_key, question_dict in
                              balanced_val_data.items()
...
def augment_qa(image_id, qa_key, question, answer, scene_graph_for_image_id, idx, produced_objects_for_image_id):
    ...
    if match_success:
        for iter in range(number_of_augmentations_to_produce):
            ...
            if (idx > DRAW_IMAGES_INDEX and qa_augmentations) or template_class_constructor == is_the_x_relation_the_y_change_relation:
            ...
            if qa_augmentations:
                ...
                if not augmented_match_success:
                    raise Exception(
                        f'Sanity - Template class did not match, idx: {idx}, qa_key: {qa_key}, image_id:{image_id}')
...
def draw_image(answer, idx, image_id, qa_augmentations, qa_key, question, scene_graph_for_image_id, template_class, iter):
    if qa_augmentations:
        ...
        if template_class_constructor == is_the_x_relation_the_y_change_relation:
            fail_case_out_d = template_class.output_dir + "_fail_cases"
            if not os.path.exists(fail_case_out_d):
                os.mkdir(fail_case_out_d)
            output_path = osp.join(fail_case_out_d,
...
def get_val_data():
    ...
    balanced_data_path = osp.join(questions_dir, "val_balanced_questions.json")
    with open(balanced_data_path) as json_file:
        balanced_val_data = json.load(json_file)
        print("Loaded {} val examples".format(len(balanced_val_data)))
    print(f"Got val data")
    return balanced_val_data, val_scene_graphs
...
if __name__ == '__main__':
    ...
    output_dir = os.path.join(experiment_output_dir, 'question_templates_outputs')
    output_csv_path = os.path.join(output_dir, 'output_files')
    if not os.path.exists(output_csv_path):
        os.mkdir(output_csv_path)
    dump_csv_steps = 10000
